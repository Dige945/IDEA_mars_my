...
class DAttentionBaseline(nn.Module):
    ...
    def forward(self, query, x, y, z, writer=None, epoch=None, img_path=None, text=''):
        ...
        reference = self._get_ref_points(H, W, B, self.ksize, self.stride, dtype, device) # [64，7，3，2]
        if self.share_offset:
...
class CDA(nn.Module):
    ...
    def forward(self, x, y, z, boss, writer=None, epoch=None, img_path=None, texts=''):
        x = x.reshape(x.size(0), self.q_size[0], self.q_size[1], -1).permute(0, 3, 1, 2) #[64, 512, 16, 8]
        ...
        x_blocks = self.split_into_blocks_with_overlap(x, self.window_size, self.stride_block).flatten(1, 2)#[64, 1,512, 16, 8]
        ...
        boss = boss.permute(0, 2, 1).unsqueeze(-2) #[64, 512, 1, 6]
        query_cash = []
