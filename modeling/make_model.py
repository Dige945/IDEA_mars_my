...
class IDEA(nn.Module):
    def __init__(self, num_classes, cfg, camera_num, view_num, factory):
        ...
        if self.DA:
            ...
            print('~~~~~~~~~~~~~~~Using CDA~~~~~~~~~~~~~~~')
        if self.direct:
            self.fusion_v = nn.Linear(3 * self.feat_dim, self.feat_dim, bias=False)
            self.fusion_v.apply(weights_init_classifier)
            self.bottleneck_fusion_v = nn.BatchNorm1d(3 * self.feat_dim)
            self.bottleneck_fusion_v.bias.requires_grad_(False)
            self.bottleneck_fusion_v.apply(weights_init_kaiming)
            
            self.classifier_v = nn.Linear(self.feat_dim, self.num_classes, bias=False)
            self.classifier_v.apply(weights_init_classifier)
            self.bottleneck_v = nn.BatchNorm1d(self.feat_dim)
